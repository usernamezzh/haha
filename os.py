...
# python3 OS文件/目录方法:os模块提供了非常丰富的方法用来处理文件和目录。
...
# os.fchown(path,uid,gid)  修改一个文件的所有权，这个函数修改一个文件的用户id和用户组id,该文件由文件描述符fd指定
...
# os.ftruncate(fd,length)  裁剪文件描述符fd对应的文件，所以它最大不超过文件大小。
# os.getcwd()  返回当前工作目录.
# os.getcwdb()  返回一个当前工作目录的unicode对象.
# os.isatty(fd)  如果文件描述符fd是打开的，同时与tty(-like)设备相连，则返回true,否则返回false.
# os.lchflags(path,flags)  设置路径的标志为数字标记，类似chflags(),但是没有软链接.
# os.lchmod(path,mode)   修改连接文件权限.
# os.lchown(path,uid,gid)  更改文件所有者，类似chown, 但是不追踪链接。
# os.link(src,dst)   创建硬链接，名为参数dst,指向参数src.
# os.listdir(path)    返回path指定的文件夹包含的文件或文件夹的名字的列表。
# os.lseek(fd,pos,how)   设置文件描述符fd当前位置为pos,how方式修改：SEEK_SET 或者 0 设置从文件开始的计算的pos; SEEK_CUR或者 1 则从当前位置计算; os.SEEK_END或者2则从文件尾部开始. 在unix，Windows中有效
# os.lstat(path)  像stat(),但是没有软链接
# os.major(device)  从原始的设备号中提取设备major号码（使用stat中的st.dev或者st.rdev field）。
